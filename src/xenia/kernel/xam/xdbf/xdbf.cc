...
uint32_t SpaFile::GetAchievements(
    XLanguage language, std::vector<Achievement>* achievements) const {
...
std::string SpaFile::GetTitleName(XLanguage language) const {
  return GetStringTableEntry(language, static_cast<uint16_t>(SpaID::Title));
}
